...
`):B=((g=m.textContent)==null?void 0:g.trim())||"";const K=B;B=B.replace(/[✅🔄⏸️🚫]/g,"").trim();let D=K.split(/\n|<br\/>|•/)[0].trim();D=D.replace(/[✅🔄⏸️🚫]/g,"").trim();const N=D.match(/^(.+?)(GET|POST|PUT|DELETE|PATCH)/);N&&(D=N[1].trim());const F=D||"Component";if(console.log("=== HOVER DEBUG ==="),console.log("Full node text:",B.substring(0,200)),console.log("Extracted node name:",D),["legend","project status","click components","completed","in progress","not started","blocked"].some(C=>D.toLowerCase().includes(C)||B.toLowerCase().includes(C)))return;const ce=[D,D+"Controller",D+"Service",D.replace(/C$/,"Controller"),D.replace(/S$/,"Service"),D.replace(/Controller$/,"")+"Controller",D.replace(/Service$/,"")+"Service",D.replace(/\s+/g,""),D.split(/\s+/)[0],D.split(/\s+/)[0]+"Controller",D.split(/\s+/)[0]+"Service"];console.log("Trying to match with IDs:",ce);let z=null,T=null;for(const C of ce)if(z=et(C),z){console.log("✅ Found node details for ID:",C),console.log("   Endpoints count:",((O=z.endpoints)==null?void 0:O.length)||0);break}for(const C of ce)if(u[C]){T=u[C],console.log("Found task info for:",C);break}!z&&!T&&console.log("No details found for node:",D,"Tried IDs:",ce);{const C=document.createElement("div");C.className="diagram-tooltip",C.style.position="fixed",window.innerWidth-re.right>500?C.style.left=`${re.right+20}px`:C.style.right=`${window.innerWidth-re.left+20}px`;const $=re.top;C.style.top=`${Math.max(10,Math.min($,window.innerHeight-400))}px`,C.setAttribute("data-tooltip","true");let j="";if(!z){let M=B.split(/\n/).map(H=>H.trim()).filter(H=>H);const ee=[];if(console.log("Raw node text:",B),console.log("Lines after split:",M),M.length===1&&((b=M[0].match(/(GET|POST|PUT|DELETE|PATCH)/g))==null?void 0:b.length)>1){console.log("Detected concatenated endpoints, splitting...");const H=M[0],le=/(GET|POST|PUT|DELETE|PATCH)\s*(\/[^\s]+?)(?=(?:GET|POST|PUT|DELETE|PATCH)|$)/g,ye=H.matchAll(le);for(const me of ye){const Re=me[2].trim();ee.push({method:me[1],path:Re}),console.log("Found endpoint:",me[1],Re)}}else M.forEach(H=>{const le=H.match(/(GET|POST|PUT|DELETE|PATCH)\s+(\/[^\s]+)/);le&&(ee.push({method:le[1],path:le[2]}),console.log("Found endpoint:",le[1],le[2]))});console.log("Total endpoints extracted:",ee.length),(ee.length>0||M.length>1)&&(z={title:F,icon:D.includes("Controller")?"🎛️":D.includes("Service")?"⚙️":D.includes("Database")||D.includes("Table")?"💾":"📦",description:`Component with ${ee.length} endpoint${ee.length!==1?"s":""}`,endpoints:ee.length>0?ee:null})}if(z){const M=z.title||F;j+=`
            ...
            `,z.endpoints&&z.endpoints.length>0&&(console.log(`Rendering ${z.endpoints.length} endpoints for ${z.title}`),j+='<div class="tooltip-section"><strong>📡 Endpoints:</strong></div>',z.endpoints.slice(0,10).forEach((H,le)=>{const ye={GET:"#10b981",POST:"#3b82f6",PUT:"#f59e0b",DELETE:"#ef4444",PATCH:"#a855f7"}[H.method]||"#6b7280";console.log(`  Endpoint ${le+1}: ${H.method} ${H.path}`),j+=`
                ...
                `}),z.endpoints.length>10&&(j+=`<div class="endpoint-desc" style="text-align: center; margin-top: 8px; font-style: italic;">... and ${z.endpoints.length-10} more</div>`)),z.services&&z.services.length>0&&(j+=`<div class="tooltip-section"><strong>⚙️ Uses:</strong> ${z.services.slice(0,5).join(", ")}</div>`),z.tables&&z.tables.length>0&&(j+=`<div class="tooltip-section"><strong>💾 Tables:</strong> ${z.tables.slice(0,5).join(", ")}</div>`),z.schema&&(j+=`<div class="tooltip-section"><strong>📋 Columns:</strong> ${Object.keys(z.schema).slice(0,6).join(", ")}</div>`)}if(T){if(!z){const le=T.title||F;j=`
                <div class="tooltip-header">
              ...
              `+j}const ee={completed:"#10b981",in_progress:"#3b82f6",blocked:"#ef4444",not_started:"#6b7280"}[T.status]||"#6b7280",H={completed:"✅",in_progress:"🔄",blocked:"🚫",not_started:"⏸️"}[T.status]||"⏸️";j+=`
